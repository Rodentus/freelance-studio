...
export class OrdersList {
    ...
    showRecords(orders) {
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            trElement.insertCell().innerHTML = orders[i].completeDate ? (new Date(orders[i].completeDate)).toLocaleString('ru-RU') : '';
            trElement.insertCell().innerHTML = CommonUtils.generateToolsColumn('orders', orders[i].id);
